  ...
  schema: {
    ...
    properties: {
      skills: {
        type: "array",
        items: { type: "string" },
        description: "Compétences ou technologies requises (ex: 'React', 'TypeScript', 'comptabilité'). Liste vide si aucune.",
      },
      seniority: {
        type: "string",
      ...
      salary_range: {
        type: ["string", "null"],
        description: "Rémunération mentionnée (ex: '450€/jour'), ou null si non précisée.",
      },
      category: {
        type: "string",
        description: "Domaine métier en 1 ou 2 mots (ex: 'Développement', 'DevOps', 'Data').",
      },
      summary: {
        type: "string",
        description: "Résumé neutre de l'offre en 2 phrases maximum, en français.",
      },
    },
    required: ["skills", "seniority", "work_mode", "salary_range", "category", "summary"],
...
async function enrichJob(job: RawJob): Promise<Enrichment> {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "Tu es un assistant spécialisé dans l'analyse et l'extraction de données structurées pour des offres d'emploi freelance.",
      },
      {
        role: "user",
...
async function main(): Promise<void> {
  ...
  const limit = Number(process.env.STRUCTURE_LIMIT);
  const jobs = Number.isFinite(limit) && limit > 0 ? allJobs.slice(0, limit) : allJobs;
